...
// SwifterJSON object will be changed as necessary by the most recent operator.
// I.e: top["title"].stringValue = "ten ways to quit" ; top["title"].intValue = 23
// will give the JSON object "title" the numeric value 23 without complaining that it used to be a string.
//
// For the subscript accessors leniency has another twist: top["title"][3].stringValue = "test" will actually create 4
// array objects. For index 0, 1, 2 and 3. However the objects at 0, 1 and 2 will contain JSON NULL objects.
// (You probably do not want to place an item at index [405764373] because your computer might run out of memory!)
// For the dictionary type subscript accessor, i.e. top["title"] the accessor will create objects for the given key if
// they don't exist. I.e. let count = top["count"].intValue will return nil, and create a JSON NULL object for the name
// "count" if it did not exist.
//
// Note: NULL objects that were created for filler purposes, but are no longer needed for that function will be
// automatically removed.
//
// It is possible to create a JSON description from any object, but only JSON descriptions from JSON OBJECTS are valid
// JSON strings.
//
...
// references. If a reference is contained in the hierarchies A and B, then changing the value in hierarchy A will also
// change the value in hierarchy B.
...
// object. When explicitly identified, JSON items are written in all capitals. I.e. as JSON object is just one of the
// possible JSON items, like STRING, NULL, NUMBER, BOOL and ARRAY.
//
...
//       Removed method 'values'
// 0.9.2 Fixed a bug that caused the 'writeJSONHierarchyToFile' to fail when the file already existed
...
// TODO: Decide if you want to keep the next typealias
...
final class SwifterJSON : CustomStringConvertible, SequenceType, Equatable {
    ...
    convenience init(_ values: Array<SwifterJSON>) {
        self.init(type: ItemType.ARRAY)
        arrayItem = []
        for value in values {
    ...
    convenience init(_ values: Dictionary<String, SwifterJSON>) {
        self.init(type: ItemType.OBJECT)
        objectItem = [:]
        for (name, value) in values {
    ...
    func removeObject(object: SwifterJSON?, forName: String?, all: Bool = false) -> Int {
        ...
        if let name = forName {
            ...
            if itemType == ItemType.OBJECT { // Btw 'object' is now guaranteed to be non-nil
                ...
                for (index, value) in arrayItem!.enumerate() {
                    if value === object! {
    ...
    var rawValue: String? { return stringItem }
    
    
    /// The string value is used, and can be retrieved, for the ItemType STRING. If 'nil' is assigned, the resulting ItemType of this SwifterJSON will be NULL.
    ...
    /// The item can be set and retrieved for ItemType ARRAY. If 'nil' is assigned, the resulting ItemType of this SwifterJSON will be NULL.
    
    var array: Array<SwifterJSON>? {
    ...
    var dictionary: Dictionary<String, SwifterJSON>? {
    ...
    /// Convenience function that maps to 'objectAtPath(path: Array<String>) -> SwifterJSON?'
    ...
    subscript(index: Int) -> SwifterJSON {
        ...
        set {
            ...
            if index >= self.arrayItem!.count {
                for _ in self.arrayItem!.count ... index {
    ...
    func values() -> Array<SwifterJSON>? {
        ...
        if itemType == ItemType.OBJECT {
            ...
            var arr = [SwifterJSON]()
            for (_, value) in objectItem! { arr.append(value) }
    ...
    struct SwifterJSONGenerator: GeneratorType {        // Implements the GeneratorType protocol
        ...
        mutating func next() -> Element? {              // The GeneratorType protocol
            if let values = source.values() {           // Only when the source has values to deliver
    ...
    // The parent SwifterJSON object (either an array or an object) or nil for the parser top level object. This value is only used during parsing and is not maintained thereafter.
    ...
    private func parse(source: String) {
        ...
        if parseError == nil {
            ...
            if activeObject == nil {
                ...
                log.atLevelDebug(source: SwifterJSON.SOURCE + ".parse", message: parseError!)
                return
            }
    ...
    private func waitForTopLevelObject(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".parseWaitForTopLevelObject", message: parseError!)
    ...
    private func waitForNameOrObjectEnd(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForNameOrObjectEnd", message: parseError!)
    ...
    private func waitForName(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForName", message: parseError!)
    ...
    private func nameEscapeSequence(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".nameEscapeSequence", message: parseError!)
    ...
    private func waitForCommaOrObjectEnd(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForCommaOrObjectEnd", message: parseError!)
    ...
    private func waitForColon(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForColon", message: parseError!)
    ...
    private func waitForValueOrArrayEnd(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForValueOrArrayEnd", message: parseError!)
    ...
    private func waitForValue(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".waitForValue", message: parseError!)
    ...
    private func valueStringEscapeSequence(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".valueStringEscapeSequence", message: parseError!)
    ...
    private func valueNumberAfterSign(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".valueNumberAfterSign", message: parseError!)
    ...
    private func valueNumberFractionStart(char: Character) -> Next {
        ...
        parseError = "Expected a number, parsing aborted at " + charLocation.description()
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".valueNumberFractionStart", message: parseError!)
    ...
    private func valueNumberStartExponent(char: Character) -> Next {
        ...
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".valueNumberStartExponent", message: parseError!)
    ...
    private func valueNumberStartExponentDigits(char: Character) -> Next {
        ...
        parseError = "Expected a number, parsing aborted at " + charLocation.description()
        log.atLevelDebug(source: SwifterJSON.SOURCE + ".valueNumberStartExponentDigits", message: parseError!)
    ...
    private func dummy(char: Character) -> Next {
        ...
        log.atLevelError(source: SwifterJSON.SOURCE + ".dummy", message: message)
    ...
    private func hexDigitsError(location: String) -> Next {
        ...
        log.atLevelError(source: SwifterJSON.SOURCE + "." + location, message: parseError!)
        return stopParsing()
    }
